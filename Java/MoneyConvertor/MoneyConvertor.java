...
public class MoneyConvertor{  
     ...
     * @throws IllegalArgumentException 如果小写金额不合法(即isLegal()返回false),无法转换,则抛出IllegalArgumentException
     */
    public MoneyConvertor(String numberStr) throws IllegalArgumentException{
    	if( !isLegal(numberStr) ){
    		...
    		this.numberString = new StringBuffer(numberStr).reverse().toString().trim();
    ...
    private void removeZero(StringBuffer resultString){  
    	if( resultString.toString().equals("零元零角") || resultString.toString().equals("零元") ){ return; }
    ...
    private void checkHead(StringBuffer resultString){  
        ...
        while( temp=='零' || temp=='元' || temp=='拾' || temp=='佰' || temp=='仟'
        		|| temp=='万' || temp=='亿' || temp=='厘' || temp=='分' || temp=='角'){
